...
describe CharacterizeJob do
  ...
  context 'when the characterization proxy content is present' do
    it 'runs CharacterizationService and creates a CreateDerivativesJob' do
      expect(CharacterizationService).to receive(:run).with(file, filename)
  ...
  context "when the file set's work is in a collection" do
    ...
    before do
      ...
      allow(CharacterizationService).to receive(:run).with(file, filename)
