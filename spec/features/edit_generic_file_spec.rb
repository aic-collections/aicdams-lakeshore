...
describe "Editing generic files" do
  ...
  context "when adding permissions" do
    ...
    it "adds departmental permissions" do
      visit sufia.edit_generic_file_path(file)
      click_link("Permissions")

      # Add the first department
      select("Department 100", from: "new_group_name_skel")
      select("View/Download", from: "new_group_permission_skel")
      click_button("add_new_group_skel")
      within("table#file_permissions") do
      ...
      select("Department 200", from: "new_group_name_skel")
