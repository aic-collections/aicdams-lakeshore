  ...
  context "with an asset" do
    ...
    describe "the create lab work button" do
      context "the asset has an imaging number" do
        ...
        it { is_expected.to have_link("Create Lab Work", href: "https://phoenix-test.artic.edu/order/create/batch/1") }
      end

      context "the asset does not have an imaging number" do
        let(:asset) { build(:asset, id: '1234', pref_label: 'FineArt') }

        it { is_expected.not_to have_link("Create Lab Work", href: "https://phoenix-test.artic.edu/order/create/batch/1") }
      end
    end
  end
