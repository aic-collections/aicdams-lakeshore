   ...
   *
   *
   * Duplicates most of Sufia permissions.js in order to add permission
   * functionalities to lists.
   *
...
Blacklight.onLoad(function() {
  ...
  $("#new_list_user_name_skel").userSearch();

  // add button for new user
  $('#add_new_list_user_skel').on('click', function() {
      if ($('#new_list_user_name_skel').val() == "" || $('#new_list_user_permission_skel :selected').index() == "0") {
        $('#new_list_user_name_skel').focus();
        return false;
      }

      if ( ($('#new_list_user_name_skel').val()+$('.add-on').text()) == $('#file_owner').data('depositor') ) {
      ...
      if (!is_permission_duplicate($('#new_list_user_name_skel').val())) {
      ...
      var user_name = $('#new_list_user_name_skel').val();
      var user_display = $('#select2-chosen-1').text();
      var access = $('#new_list_user_permission_skel').val();
  ...
  // add button for new user
  $('#add_new_list_group_skel').on('click', function() {
      if ($('#new_list_group_name_skel :selected').index() == "0" || $('#new_list_group_permission_skel :selected').index() == "0") {
        $('#new_list_group_name_skel').focus();
        return false;
      }
      var group_name = $('#new_list_group_name_skel').val();
      var group_display = $("#new_list_group_name_skel option[value='"+group_name+"']").text();
      var access = $('#new_list_group_permission_skel').val();
      var access_label = 'Edit';

      if (!is_permission_duplicate($('#new_list_group_name_skel').val())) {
  ...
  $("input[name='visibility']").on("change", set_access_levels);
	...
	$('#list_permissions_new_list_group_name').change(function (){
      var edit_option = $("#list_permissions_new_list_group_permission option[value='edit']")[0];
  ...
  function addUserPerm(agent_name, access, access_label, agent_display)
  {
      ...
      $('#list_permissions').find('tbody:last').append(tr);
      tr.effect("highlight", {}, 3000);
  }

  function addGroupPerm(agent_name, access, access_label, agent_display)
  {
      showPermissionNote();

      var tr = createPermissionRow(agent_display, access_label);
      addHiddenPermField(tr, 'group', agent_name, access);
      $('#list_permissions').find('tbody:last').append(tr);
      tr.effect("highlight", {}, 3000);
  }
  ...
  function addHiddenPermField(element, type, name, access) {
      var prefix = 'list[permissions_attributes][' + nextIndex() + ']';
      $('<input>').attr({
          type: 'hidden',
          name: prefix + '[type]',
          value: type
  ...
  function nextIndex() {
      return $('#list_permissions').find('tbody').children().size();
  }
  ...
  function addDestroyField(element, index) {
      $('<input>').attr({
          type: 'hidden',
          name: 'list[permissions_attributes][' + index + '][_destroy]',
          value: 'true'
      }).appendTo(element);
  }
...
function set_access_levels() {
  ...
  $('#new_list_group_permission_skel option[value=read]').attr("disabled", enabled_disabled);
  $('#new_list_user_permission_skel option[value=read]').attr("disabled", enabled_disabled);
  var perms_sel = $("select[name^='list[permissions]']");
...
function is_permission_duplicate(user_or_group_name)
{
  ...
  var perms_input = $("input[name^='list[permissions]']");
  var perms_sel = $("select[name^='list[permissions]']");
  var flag = 1;
