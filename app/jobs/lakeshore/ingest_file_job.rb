...
# Overrides IngestFileJob in CurationConcerns to perform all ingest functions in one job
# on a custom queue.
class Lakeshore::IngestFileJob < ActiveJob::Base
  queue_as :api
  ...
  def perform(file_set, filepath, user, opts = {})
    ...
    Lakeshore::CreateAllDerivatives.perform_later(file_set, repository_file.id, filepath)
  end
end
